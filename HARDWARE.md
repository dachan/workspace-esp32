...
- Rotary encoders (×2 KY-040-style) on the **RIGHT** header of `s3-n16r8.jpeg` (left header is display SPI). Both `+`→3V3 / `GND`→GND (right-side GND pins OK):
  - **Thinking** — CLK GPIO41, DT GPIO40, SW GPIO39 (Instant→Medium→High→Extra High; click = next)
  - **Model** — CLK GPIO1, DT GPIO2, SW GPIO42 (clamps first↔last preset; click = next, no wrap)
  - Changes persist in NVS and emit `SET MODEL` / `SET THINKING` to the Mac bridge
  - Board pinout image: [`s3-n16r8.jpeg`](s3-n16r8.jpeg)
- Touch: not used by this firmware (FT6336 may be present)
- Protocol: USB Serial/JTAG 115200. Mac→ESP `MODEL <name>` / `THINKING <level>`; ESP→Mac `SET MODEL <name>` / `SET THINKING <level>`
...
- Display settings (LOCKED — verified against desk photo 2026-09-06):
  ...
  - MADCTL: `swap_xy(true)`, `mirror(true, true)` — **do not change**
  - Software 180 flush: ON (with MADCTL mirror true,true)
...
- Last verified: 2026-09-06 — upright match to desk photo; segmented thinking bars
