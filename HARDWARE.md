...
- Firmware: flashed image is `ai-model-control/ESP32_MINI-128_tft_240x240-AI_Model_Control-New` from commit `05cddcf`; display order RST/CS/DC/SDA/SCL -> GPIO8/9/10/11/12, Model encoder direction `-1`, readable display rotation `180` degrees, with 400 ms panel settle, 200 ms bridge settle and picker/confirmation waits, and a 50 ms posted-key gap.
- Mac USB serial: `/dev/cu.usbmodem21201` (re-verify after replug; macOS may renumber native USB CDC ports)
- Last verified: 2026-09-15 — application-only reflash from commit `05cddcf` to `/dev/cu.usbmodem21201`, preserving NVS; esptool verified the written application data. Chip matched ESP32-S3 QFN56 rev v0.2 with embedded 4 MB flash + 2 MB PSRAM, MAC `d4:05:92:47:d5:1c`. Firmware SHA-256 `c004fba0484c7190a40179459129476e9473a81d21c3fb7dd6ca323c7a4093`. Post-reset serial repeatedly announced READY boot id `bc8512ac000e6285` until the restarted Model Dial bridge sent SYNC.
