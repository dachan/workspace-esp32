...
- Last verified: 2026-09-15 — chip, 16 MB flash, and 8 MB PSRAM matched MAC `7c:4f:ad:ba:c8:98`; bootloader, partition table, and application from commit `f78a550` each passed independent esptool verification. The boot log reached both display-ready messages and reported the NTP clock sync feature ready for USB-serial Wi-Fi provisioning. The TFT renders the fluid-motion screensaver without an on-screen frame-rate counter. The OLED renders a centered `HH:MM` time above the centered date; serial status retains second precision, and both text lines follow an eight-position ±2-pixel shift pattern advancing every 30 minutes. The physical panel appearance and live NTP response remain unverified.
