...
- Firmware: flashed image is `ai-model-control/ESP32_MINI-128_tft_240x240-AI_Model_Control-New` from commit `f80ce06`; display order RST/CS/DC/SDA/SCL -> GPIO8/9/10/11/12, Model encoder direction `-1`, readable display rotation `180` degrees. Dial rotation applies after the 400 ms firmware settle; either encoder click explicitly pushes the current model and effort, including a press captured on the same polling cycle as a focus transition. Passive focus and connection snapshots update state without posting keys. Picker/confirmation waits remain 100 ms, with a 50 ms posted-key gap.
- Mac USB serial: `/dev/cu.usbmodem21201` (re-verify after replug; macOS may renumber native USB CDC ports)
- Last verified: 2026-09-15 — application-only reflash from commit `f80ce06` to `/dev/cu.usbmodem21201`, preserving NVS; esptool verified the written application data. Chip matched ESP32-S3 QFN56 rev v0.2 with embedded 4 MB flash + 2 MB PSRAM, MAC `d4:05:92:47:d5:1c`. Firmware SHA-256 `b3aee05ff81f845870930fca9e741f2888e2892072df8afd4fd8b9c98b4c00ed`. Boot log reported app version `f80ce06`, ESP-IDF v6.0.2, successful PSRAM testing, and repeatedly announced READY boot id `1e28a3f6a4c2720e` for 35 seconds until the restarted Model Dial bridge sent SYNC. The signed installed helper carries the same firmware hash and is connected to the board.
