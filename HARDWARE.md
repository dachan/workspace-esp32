...
Last inventory pass: 2026-09-07 (chatgpt-model-display v0.68 flash)
...
- Firmware currently on device: `chatgpt-model-display/` **v 0.68** (last model per app on FRONT switch; SYNC tap force-applies the displayed pair)
...
- Last verified: 2026-09-07 — v0.68 `app-flash` on `/dev/cu.usbmodem21201` (hash-verified); rebuilt bridge reconnected and read back the NVS dial state; FRONT restores each app's last model and effort; SYNC tap force-applies the displayed pair; model CLK/DT wiring fault still open (see above)
