...
| New round ai-model-control SuperMini | MAC `d4:05:92:47:d5:1c`, ESP32-S3 QFN56 rev v0.2, 4 MB flash / 2 MB PSRAM, display wired `RST/CS/DC/SDA/SCL -> GPIO8/9/10/11/12` | `ai-model-control/build-supermini-new-order-reversed-model` | Original round build |
...
- Firmware: expected image is `ai-model-control/build-supermini-new-order-reversed-model`; display order RST/CS/DC/SDA/SCL -> GPIO8/9/10/11/12 and Model encoder direction `1` (the reverse of the prior new-board image). The earlier flashed image remains `build-supermini-new-order` until replaced.
- Mac USB serial: `/dev/cu.usbmodem21201` (re-verify after replug; macOS may renumber native USB CDC ports)
- Last verified: 2026-09-13 — full flash of `ai-model-control/build-supermini-new-order` from commit `d7dc05c` to `/dev/cu.usbmodem21201` (hash verified by esptool). Chip matched ESP32-S3 QFN56 rev v0.2 with embedded 4 MB flash + 2 MB PSRAM, MAC `d4:05:92:47:d5:1c`. Firmware SHA-256 `333512c0b2f9daf9c1f29a3d164e81424519ed2c64800cd1fd042c2ad613c8bf`. Boot serial repeated `READY ca2f71102f48badb` after reset
