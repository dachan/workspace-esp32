...
- Last verified: 2026-09-21 — application-only reflash of `ESP32_MINI-128_tft_240x240-AI_Model_Control-New` v0.90 to `/dev/cu.usbmodem21201`, preserving NVS; esptool verified the write and an independent readback. Chip matched ESP32-S3 QFN56 rev v0.2 with embedded 4 MB flash + 2 MB PSRAM, MAC `d4:05:92:47:d5:1c`. Firmware from commit `70e37f3` has SHA-256 `8d8c5fa24bad9fcff7405f09347021efef84f212627595179f3a0d27e123956a`; its boot log reached `READY`. The signed Model Dial bridge reconnected with Cursor mask `00000020000044d1`, including Grok 4.7; the firmware traversal places Grok 4.7 immediately after Auto without changing that mask's slot positions. Physical panel appearance remains unverified. Rig models walk Z–A by the full provider-plus-name label. Default knobs are left = model (GPIO4/5/6) and right = effort (GPIO1/2/7); Settings → Dials can swap them.
