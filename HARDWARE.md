...
- Last verified: 2026-09-18 — application-only reflash of `ESP32_MINI-128_tft_240x240-AI_Model_Control-New` v0.90 to `/dev/cu.usbmodem21201`, preserving NVS; esptool verified the written application data. Chip matched ESP32-S3 QFN56 rev v0.2 with embedded 4 MB flash + 2 MB PSRAM, MAC `d4:05:92:47:d5:1c`. Firmware SHA-256 `0aced66275b3105c5155de1ecd80168936041fe3746af023569ace7b1be1b855`. Rig panel names follow OpenRouter `Provider: Model` titles (drop the prefix, then a duplicated provider token, same as the composer). The encoder still cycles Latest aliases; any other enabled model is shown from host `MODEL`. Focusing Rig sends `FRONT Rig` plus host `MODEL`/`THINKING` from `agent.focus`.
