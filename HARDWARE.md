...
Last inventory pass: 2026-09-07 (chatgpt-model-display v0.70 flash)
...
- Firmware currently on device: `chatgpt-model-display/` **v 0.70** (date+time on one header line; larger orange SYNC; last model per app on FRONT switch)
...
- Last verified: 2026-09-07 — v0.70 `app-flash` on `/dev/cu.usbmodem21201` (hash-verified); bridge reconnected; date and time share the header line at matching type size; SYNC is larger orange; FRONT restores each app's last model and effort; model CLK/DT wiring fault still open (see above)
