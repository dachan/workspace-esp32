...
// 5 columns x 7 rows, LSB = top pixel. Printable ASCII 32..126.
...
void font_draw_char(int x, int y, char c, uint16_t fg, uint16_t bg, int scale)
{
    ...
    for (int col = 0; col < 5; col++) {
        ...
        for (int row = 0; row < 7; row++) {
            ...
            display_fill_rect(x + col * scale, y + row * scale, scale, scale, colour);
        }
    }
}
...
int font_text_width(const char *text, int scale)
{
    ...
    return n * 6 * scale - scale;  // 5px glyph + 1px gap
}
